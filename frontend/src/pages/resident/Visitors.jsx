...
import { getVisitors, respondToVisitor, preApproveVisitor } from '../../api/visitorApi';
...
const ResidentVisitors = () => {
  ...
  const [tab, setTab] = useState('requests'); // 'requests' | 'create invite'
  const [inviteData, setInviteData] = useState({ name: '', phone: '', purpose: 'guest' });
  ...
  const handleCreateInvite = async (e) => {
    ...
    try {
      const res = await preApproveVisitor(inviteData);
      setGeneratedPin(res.data.data.otp.code);
      toast.success('Invite generated successfully!');
      fetchVisitors();
      setInviteData({ name: '', phone: '', purpose: 'guest' });
    } catch (err) {
      ...
      <div className="space-y-6">
                    ...
                    {history.map((v) => (
                      <div key={v._id} className="flex items-center gap-4 p-4 hover:bg-dark-700/20">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500">{v.phone} · {v.preApproved ? 'Pre-Approved' : v.purpose} · {new Date(v.createdAt).toLocaleDateString()}</p>
                        </div>
        ...
        {tab === 'create invite' && (
          <div className="glass-card p-6 max-w-lg mx-auto">
            <h2 className="text-xl font-bold text-white mb-4">Create Visitor Invite</h2>
            <p className="text-slate-400 text-sm mb-6">Pre-approve a visitor so they can fast-track at the gate with a 6-digit PIN.</p>
            ...
            <form onSubmit={handleCreateInvite} className="space-y-4">
              ...
              <div>
                ...
                <input 
                  ...
                  className="input-field w-full" placeholder="10-digit number"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Purpose</label>
                <select 
                  value={inviteData.purpose} onChange={(e) => setInviteData({...inviteData, purpose: e.target.value})}
              ...
              <button type="submit" disabled={inviteLoading} className="btn-primary w-full mt-4">
                {inviteLoading ? 'Generating...' : 'Generate Entry PIN'}
              </button>
            </form>

            {generatedPin && (
              <div className="mt-8 p-6 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-center animate-fade-in">
                <UserCheck className="w-12 h-12 text-emerald-500 mx-auto mb-2" />
                <h3 className="text-lg font-bold text-white">Invite Created!</h3>
                <p className="text-slate-400 text-sm mb-4">Share this PIN with your visitor for Fast-Track entry at the gate.</p>
                <div className="text-4xl font-mono font-black text-emerald-400 tracking-widest bg-dark-900 py-4 rounded-xl border border-dark-700">
                  {generatedPin}
                </div>
                <button 
                  onClick={() => {
                    navigator.clipboard.writeText(`Hi! Use PIN ${generatedPin} for fast-track entry at the gate.`);
